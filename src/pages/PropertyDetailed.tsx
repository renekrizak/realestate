import React, { useState } from "react";
import { PropertyFull } from "../data/ListingsData";
import "../style/compStyles/PropertyDetailed.scss";

import Navbar from "../components/navbar";
import { Footer } from "../components/TestimonialsFooter";

interface ListingDetailProps {
  property: PropertyFull;
}

const PropertyDetailed: React.FC<ListingDetailProps> = ({ property }) => {
  const [slideshowActive, setSlideshowActive] = useState<boolean>(false);
  const [slideshowIndex, setSlideshowIndex] = useState(0);
  const handleOpenSlideshow = () => {
    setSlideshowActive(() => true);
  };
  const handleCloseSlideshow = () => {
    setSlideshowActive(() => false);
  };

  const goToNextSlide = () => {
    setSlideshowIndex((prevIndex) =>
      prevIndex === property.allImages.length - 1 ? 0 : prevIndex + 1
    );
  };
  const goToPrevSlide = () => {
    setSlideshowIndex((prevIndex) =>
      prevIndex === 0 ? property.allImages.length - 1 : prevIndex - 1
    );
  };

  const changeImageIndex = (index: number) => {
    setSlideshowIndex(index);
  };
  const showcasedImages = property.allImages.slice(1, 5);
  return (
    <>
      <div
        className={
          slideshowActive === true
            ? "property-slideshow-full property-enabled"
            : "property-disabled"
        }
      >
        <div className="property-slideshow-control">
          <svg
            onClick={handleCloseSlideshow}
            xmlns="http://www.w3.org/2000/svg"
            height="1em"
            viewBox="0 0 384 512"
          >
            <path d="M376.6 84.5c11.3-13.6 9.5-33.8-4.1-45.1s-33.8-9.5-45.1 4.1L192 206 56.6 43.5C45.3 29.9 25.1 28.1 11.5 39.4S-3.9 70.9 7.4 84.5L150.3 256 7.4 427.5c-11.3 13.6-9.5 33.8 4.1 45.1s33.8 9.5 45.1-4.1L192 306 327.4 468.5c11.3 13.6 31.5 15.4 45.1 4.1s15.4-31.5 4.1-45.1L233.7 256 376.6 84.5z" />
          </svg>
        </div>
        <div className="property-slideshow-main-image">
          <div className="slideshow-main-image-control">
            <svg
              onClick={goToPrevSlide}
              xmlns="http://www.w3.org/2000/svg"
              height="1em"
              viewBox="0 0 320 512"
            >
              <path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l192 192c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L77.3 256 246.6 86.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192z" />
            </svg>
          </div>
          <div className="slideshow-current-image">
            <img src={property.allImages[slideshowIndex]} alt="" />
          </div>
          <div className="slideshow-main-image-control">
            <svg
              onClick={goToNextSlide}
              xmlns="http://www.w3.org/2000/svg"
              height="1em"
              viewBox="0 0 320 512"
            >
              <path d="M310.6 233.4c12.5 12.5 12.5 32.8 0 45.3l-192 192c-12.5 12.5-32.8 12.5-45.3 0s-12.5-32.8 0-45.3L242.7 256 73.4 86.6c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l192 192z" />
            </svg>
          </div>
        </div>
        <div className="property-slideshow-all-images">
          {property.allImages.map((image, index) => (
            <div key={index} className="img-slider">
              <img
                src={image}
                alt={`Image number ${index}`}
                onClick={() => changeImageIndex(index)}
              />
            </div>
          ))}
        </div>
      </div>
      <div
        className={
          slideshowActive === false
            ? "property-content property-enabled"
            : "property-disabled"
        }
      >
        <Navbar></Navbar>
        <section className="section-property-detail">
          <div className="property-detail-container">
            <div className="property-detail-address">
              <h2>{property.address}</h2>
              <h3>{property.shortDesc}</h3>
            </div>
            <div className="property-detail-images">
              <div className="detail-main-image">
                <img src={property.image} alt={property.shortDesc} />
                <div className="image-overlay">
                  <div className="overlay-content">
                    <button onClick={handleOpenSlideshow}>
                      Show all images
                    </button>
                  </div>
                </div>
              </div>
              <div className="detail-small-images">
                {showcasedImages.map((image, index) => (
                  <img key={index} src={image} alt={`Image ${index}`} />
                ))}
              </div>
            </div>
            <div className="property-main-text">
              <div className="property-main-text-content">
                <h2>{property.longDesc}</h2>
                <div className="property-main-amenities">
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 640 512"
                    >
                      <path d="M32 32c17.7 0 32 14.3 32 32V320H288V160c0-17.7 14.3-32 32-32H544c53 0 96 43 96 96V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V416H352 320 64v32c0 17.7-14.3 32-32 32s-32-14.3-32-32V64C0 46.3 14.3 32 32 32zm144 96a80 80 0 1 1 0 160 80 80 0 1 1 0-160z" />
                    </svg>
                    {property.bedrooms} bedrooms
                  </div>
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 512 512"
                    >
                      <path d="M96 77.3c0-7.3 5.9-13.3 13.3-13.3c3.5 0 6.9 1.4 9.4 3.9l14.9 14.9C130 91.8 128 101.7 128 112c0 19.9 7.2 38 19.2 52c-5.3 9.2-4 21.1 3.8 29c9.4 9.4 24.6 9.4 33.9 0L289 89c9.4-9.4 9.4-24.6 0-33.9c-7.9-7.9-19.8-9.1-29-3.8C246 39.2 227.9 32 208 32c-10.3 0-20.2 2-29.2 5.5L163.9 22.6C149.4 8.1 129.7 0 109.3 0C66.6 0 32 34.6 32 77.3V256c-17.7 0-32 14.3-32 32s14.3 32 32 32H480c17.7 0 32-14.3 32-32s-14.3-32-32-32H96V77.3zM32 352v16c0 28.4 12.4 54 32 71.6V480c0 17.7 14.3 32 32 32s32-14.3 32-32V464H384v16c0 17.7 14.3 32 32 32s32-14.3 32-32V439.6c19.6-17.6 32-43.1 32-71.6V352H32z" />
                    </svg>
                    {property.bathrooms} bathrooms
                  </div>
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 640 512"
                    >
                      <path d="M54.2 202.9C123.2 136.7 216.8 96 320 96s196.8 40.7 265.8 106.9c12.8 12.2 33 11.8 45.2-.9s11.8-33-.9-45.2C549.7 79.5 440.4 32 320 32S90.3 79.5 9.8 156.7C-2.9 169-3.3 189.2 8.9 202s32.5 13.2 45.2 .9zM320 256c56.8 0 108.6 21.1 148.2 56c13.3 11.7 33.5 10.4 45.2-2.8s10.4-33.5-2.8-45.2C459.8 219.2 393 192 320 192s-139.8 27.2-190.5 72c-13.3 11.7-14.5 31.9-2.8 45.2s31.9 14.5 45.2 2.8c39.5-34.9 91.3-56 148.2-56zm64 160a64 64 0 1 0 -128 0 64 64 0 1 0 128 0z" />
                    </svg>
                    High Speed Internet
                  </div>
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 512 512"
                    >
                      <path d="M135.2 117.4L109.1 192H402.9l-26.1-74.6C372.3 104.6 360.2 96 346.6 96H165.4c-13.6 0-25.7 8.6-30.2 21.4zM39.6 196.8L74.8 96.3C88.3 57.8 124.6 32 165.4 32H346.6c40.8 0 77.1 25.8 90.6 64.3l35.2 100.5c23.2 9.6 39.6 32.5 39.6 59.2V400v48c0 17.7-14.3 32-32 32H448c-17.7 0-32-14.3-32-32V400H96v48c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V400 256c0-26.7 16.4-49.6 39.6-59.2zM128 288a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm288 32a32 32 0 1 0 0-64 32 32 0 1 0 0 64z" />
                    </svg>
                    Private parking
                  </div>
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 384 512"
                    >
                      <path d="M215.7 499.2C267 435 384 279.4 384 192C384 86 298 0 192 0S0 86 0 192c0 87.4 117 243 168.3 307.2c12.3 15.3 35.1 15.3 47.4 0zM192 128a64 64 0 1 1 0 128 64 64 0 1 1 0-128z" />
                    </svg>
                    Great location
                  </div>
                  <div className="property-main-amenity">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      height="1em"
                      viewBox="0 0 576 512"
                    >
                      <path d="M128 127.7C128 74.9 170.9 32 223.7 32c48.3 0 89 36 95 83.9l1 8.2c2.2 17.5-10.2 33.5-27.8 35.7s-33.5-10.2-35.7-27.8l-1-8.2c-2-15.9-15.5-27.8-31.5-27.8c-17.5 0-31.7 14.2-31.7 31.7V224H384V127.7C384 74.9 426.9 32 479.7 32c48.3 0 89 36 95 83.9l1 8.2c2.2 17.5-10.2 33.5-27.8 35.7s-33.5-10.2-35.7-27.8l-1-8.2c-2-15.9-15.5-27.8-31.5-27.8c-17.5 0-31.7 14.2-31.7 31.7V361c-1.6 1-3.3 2-4.8 3.1c-18 12.4-40.1 20.3-59.2 20.3h0V288H192v96.5c-19 0-41.2-7.9-59.1-20.3c-1.6-1.1-3.2-2.2-4.9-3.1V127.7zM306.5 389.9C329 405.4 356.5 416 384 416c26.9 0 55.4-10.8 77.4-26.1l0 0c11.9-8.5 28.1-7.8 39.2 1.7c14.4 11.9 32.5 21 50.6 25.2c17.2 4 27.9 21.2 23.9 38.4s-21.2 27.9-38.4 23.9c-24.5-5.7-44.9-16.5-58.2-25C449.5 469.7 417 480 384 480c-31.9 0-60.6-9.9-80.4-18.9c-5.8-2.7-11.1-5.3-15.6-7.7c-4.5 2.4-9.7 5.1-15.6 7.7c-19.8 9-48.5 18.9-80.4 18.9c-33 0-65.5-10.3-94.5-25.8c-13.4 8.4-33.7 19.3-58.2 25c-17.2 4-34.4-6.7-38.4-23.9s6.7-34.4 23.9-38.4c18.1-4.2 36.2-13.3 50.6-25.2c11.1-9.4 27.3-10.1 39.2-1.7l0 0C136.7 405.2 165.1 416 192 416c27.5 0 55-10.6 77.5-26.1c11.1-7.9 25.9-7.9 37 0z" />
                    </svg>
                    Pool access
                  </div>
                </div>
              </div>
              <div className="property-contact-form">
                <form>
                  <h3>Schedule tour with our agent!</h3>
                  <input
                    type="Text"
                    name="Full name"
                    placeholder="Your full name"
                  />
                  <input
                    type="text"
                    name="Buy/Rent"
                    placeholder="Interested in buying or renting?"
                  />
                  <input
                    type="text"
                    name="contact phone"
                    placeholder="Phone Number"
                  />
                  <input
                    type="text"
                    name="contact email"
                    placeholder="Email address"
                  />
                  <button>Submit</button>
                </form>
              </div>
            </div>
          </div>
        </section>
        <Footer></Footer>
      </div>
    </>
  );
};

export default PropertyDetailed;
