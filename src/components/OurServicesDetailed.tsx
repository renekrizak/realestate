...
const serviceData: Record<string, ServiceList[]> = {
  Buy: [
    {
...
function isServiceList(item: ServiceList): item is ServiceList {
  ...
  return (
    <div className="services-detailed">
      <div className="services-detailed-selectors">
        <ul>
          {Object.keys(serviceData).map((option) => (
            <li
              key={option}
      ...
      <div className="services-detailed-text-agent">
        {serviceData[currentSelection]?.map(
          (item, index) =>
            isServiceList(item) && (
              <>
                <div className="services-detailed-text" key={index}>
                  <div key={index}>
                    <h1>{item.title}</h1>
                    <p>{item.text}</p>
                  </div>
